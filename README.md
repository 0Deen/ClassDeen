...
At ClassDeen, our mission is to provide students with the best possible learning experience. We believe in empowering educators with the tools they need to deliver high-quality education that is accessible, engaging, and effective.
...
4. **Run Courses on Messengers:** Students can just join a channel on slack/discord/telegram and a bot 

automatically sends daily lesson content to your students without you doing anything.
## Built With
...
3. `dashboard`: The web application that runs the learning management system hosted [here](https://app.classDeen.com).
4. `docs`: Official documentation of ClassroomIO hosted [here](https://classDeen.com/docs)
...
1. Fork the repo, then clone it using the following command (remember to replace the url with the url from your forked repo)
   ...
   ```bash
   git clone https://github.com/0Deen/classDeen.git
   ```
...
4. Set up your `.env` file
   ...
   - Go to `apps/classDeen-com`, `apps/dashboard` and `apps/backend`.
